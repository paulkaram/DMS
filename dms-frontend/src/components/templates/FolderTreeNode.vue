...
<template>
  <div>
    <div
      class="flex items-center gap-2 py-2 px-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 group"
      :style="{ marginLeft: (depth || 0) * 24 + 'px' }"
    >
      <span class="material-symbols-outlined text-amber-500">folder</span>
      <span class="font-medium text-slate-700 dark:text-slate-200">{{ node.name }}</span>
      <span
      ...
      <div class="ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          ...
          class="p-1 text-slate-400 hover:text-teal rounded"
          title="Add subfolder"
        >
